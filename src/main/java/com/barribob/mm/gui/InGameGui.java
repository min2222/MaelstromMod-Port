...
import com.barribob.mm.items.gun.Reloadable;
...
public class InGameGui {
        ...
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.ALL) && mc.playerController.shouldDrawHUD()) {
            ...
            if (maelstromArmor > 0) {
                for (int i = 0; i < 10; i++) {
                    ...
                    int framesPerTick = 3;
                    int y = 9 * (Math.floorDiv(player.tickCount, framesPerTick) % animationLength);
    ...
    public static void renderManaBar(Minecraft mc, RenderGameOverlayEvent.Post event, Player player) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.ALL) && mc.playerController.shouldDrawHUD()) {
            ...
            int frame = Math.floorDiv(player.tickCount, framesPerTick) % frames.length;
