...
public class ProjectileMegaFireball extends ProjectileAbstractMegaFireball {
    ...
    protected void spawnParticles() {
        for (int i = 0; i < PARTICLE_AMOUNT; i++) {
            ...
            ParticleManager.spawnEffect(level, origin.add(ModRandom.randVec()), ModColors.FIREBALL_ORANGE);
        }
    }
    ...
    protected void spawnImpactParticles() {
        for (int i = 0; i < IMPACT_PARTICLE_AMOUNT; i++) {
            ...
            Vec3 vel = pos.subtract(this.position()).normalize().scale(level.random.nextFloat() * 0.3f);
    ...
    protected void onImpact(@Nullable HitResult result) {
        ...
        boolean flag = net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.level, this.shootingEntity);
        if(!level.isClientSide) {
            this.level.explode(null, this.getX(), this.getY(), this.getZ(), 3, true, flag ? BlockInteraction.DESTROY : BlockInteraction.NONE);

            for (int i = 0; i < 10; i++) {
                Vec3 vel = ModRandom.randVec().normalize().scale(0.5f).add(ModUtils.yVec(1));
                ProjectileFireball shrapenel = new ProjectileFireball(level, shootingEntity, this.getDamage() * 0.5f, null);
