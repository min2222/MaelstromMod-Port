...
public class BlockDecayingMaelstrom extends BlockLeavesBase {
    ...
    public void onEntityCollidedWithBlock(Level worldIn, BlockPos pos, BlockState state, Entity entityIn) {
        if (entityIn instanceof LivingEntity && !EntityMaelstromMob.isMaelstromMob(entityIn)) {
            entityIn.hurt(ModDamageSource.MAELSTROM_DAMAGE, damage);
        }
    }

    @Override
    @OnlyIn(Dist.CLIENT)
    public void randomTick(BlockState stateIn, Level worldIn, BlockPos pos, RandomSource rand) {
        for (int i = 0; i < 3; i++) {
    ...
    @Override
    public void tick(Level worldIn, BlockPos pos, BlockState state, Random rand) {
        if (!worldIn.isRemote) {
