        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (!player.level.isClientSide && player instanceof ServerPlayer && (heldItem.equals(ModItems.CROSS_OF_AQUA) || offhandItem.equals(ModItems.CROSS_OF_AQUA))) {
                ...
                if (!mana.isLocked() && mana.getMana() > 0) {
                    if (!player.capabilities.isCreativeMode && player.tickCount % 40 == 0) {
                        mana.consume(1);
            ...
            if (bonusConfig.getBoolean("nyan") &&
                ...
                Vec3 pos = player.position().subtract(new Vec3(player.getLookVec().x, 0, player.getLookVec().z));
            ...
            if (player.level.isClientSide && chestplate.equals(ModItems.ELYSIUM_WINGS) && player.isElytraFlying()) {
                double speed = Math.max(0.1, Math.min(1, new Vec3(player.motionX, 0, player.motionZ).lengthVector() + player.motionY));
                ParticleManager.spawnFirework(player.world, player.getEyePosition(1).add(ModUtils.getRelativeOffset(player, new Vec3(0, 0, -speed))), ModColors.RED);
                ParticleManager.spawnFirework(player.world, player.getEyePosition(1).add(ModUtils.getRelativeOffset(player, new Vec3(0, 0, speed))), ModColors.RED);
            }
        }
    }
